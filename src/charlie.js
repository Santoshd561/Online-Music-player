...
import './Artist.css';
import charlie from './photoco/charlie.jpg'
...
export default class Charlie extends Component {
  ...
  componentDidMount() {
    fetch("http://localhost:5000/charlie_songs")
      ...
      <Media>
        <div className="wrapper">
          ...
          <h1>Charlie Songs</h1>
            ...
            {this.state.songs_list.map((s) => (
              <ul>
                <button className="button" style={{marginLeft:"-400px",marginTop:"-20px"}}
                  onClick={() => {
                    this.setState({
                      song: "http://localhost:5000/getcharlie_song?song_name=" + s.name
                    })
                  }}>
                  {s.name}
                </button>
            ...
            <div>
                <img src={charlie} class="card-img-top" alt="h" style={{ width: "340px", height: "340px",marginLeft:"480px",marginTop:"-320px" }} />
                </div>
