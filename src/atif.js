...
import './Artist.css';
import atif from './photoco/atif.jpg'
...
export default class Atif extends Component {
  ...
  componentDidMount() {
    fetch("http://localhost:5000/atif_songs")
      ...
      <Media>
        <div className="wrapper">
          ...
          <h1>Atif Songs</h1>
            ...
            {this.state.songs_list.map((s) => (
              <ul>
                <button className="button" style={{marginLeft:"-400px",marginTop:"-20px"}}
                  onClick={() => {
                    this.setState({
                      song: "http://localhost:5000/getatif_song?song_name=" + s.name
                    })
                  }}>
                  {s.name}
                </button>
            ...
            <div>
                <img src={atif} class="card-img-top" alt="h" style={{ width: "340px", height: "340px",marginLeft:"480px",marginTop:"-320px" }} />
                </div>
