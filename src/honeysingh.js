...
import './Artist.css';
import honey from './photoco/honey.jpg';
...
export default class Honey extends Component {
  ...
  componentDidMount() {
    fetch("http://localhost:5000/honey_songs")
      ...
      <Media>
        <div className="wrapper">
          ...
          <h1>HoneySingh Songs</h1>
            ...
            {this.state.songs_list.map((s) => (
              <ul>
                <button className="button" style={{marginLeft:"-400px",marginTop:"-20px"}}
                  onClick={() => {
                    this.setState({
                      song: "http://localhost:5000/gethoney_song?song_name=" + s.name
                    })
                  }}>
                  {s.name}
                </button>
            ...
            <div>
                <img src={honey} class="card-img-top" alt="h" style={{ width: "340px", height: "340px",marginLeft:"480px",marginTop:"-320px" }} />
                </div>
