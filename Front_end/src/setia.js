...
import './Artist.css';
import setia from './photoco/setia.jpg'



export default class Setia extends Component {
  constructor(props) {
    super(props);
    this.state = {
      songs_list: [],
  ...
  componentDidMount() {
    fetch("http://localhost:5000/setia_songs")
      ...
      <Media>
        <div className="wrapper">
          ...
          <h1>Setia Songs</h1>
            ...
            {this.state.songs_list.map((s) => (
              <ul>
                <button className="button" style={{marginLeft:"-400px",marginTop:"-20px"}}
                  onClick={() => {
                    this.setState({
                      song: "http://localhost:5000/getsetia_song?song_name=" + s.name
                    })
                  }}>
                  {s.name}
                </button>
            ...
            <div>
                <img src={setia} class="card-img-top" alt="h" style={{ width: "340px", height: "340px",marginLeft:"480px",marginTop:"-320px" }} />
                </div>
